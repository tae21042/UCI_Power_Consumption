plot4 <- function(){
    ...
    par(mfcol=c(2,2))
    plot(x,data.names$Global_active_power,"l",
    ...
    legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),cex=.7,
           text.width =75000,lty="solid", col=c("black","red","blue"),bty="n")
    plot(x,data.names$Voltage,"l",xlab="datetime",ylab="Voltage")
    plot(x,data.names$Global_reactive_power,"l",xlab="datetime",ylab="Global_reactive_power")
    
    dev.copy(png,file="./4_Exploratory/plot4.png",width=480,height=480)
    dev.off()
